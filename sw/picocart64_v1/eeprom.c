...
uint8_t eeprom[EEPROM_4KBIT_SIZE];	//EEPROM_16KBIT_SIZE

// eeprom_backup will be aligned to 4096 to match the flash sector erase size
uint8_t __aligned(4096) __attribute__((section(".n64_eeprom"))) eeprom_backup[EEPROM_4KBIT_SIZE];   //EEPROM_16KBIT_SIZE
...
void eeprom_save_to_flash(void)
{
	...
	uint32_t count = 4096;  //sizeof(eeprom) is smaller than sector size (4k)
